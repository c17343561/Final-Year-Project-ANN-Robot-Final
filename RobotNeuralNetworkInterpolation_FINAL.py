...
# network on interpolated so the 0mm distances are filled with estimated values
# instead.
...
import pandas as pd # Panda's series for interpolation data structure
...
############################################################################
checkpointPath = "training_Interpolated_test/cp-{epoch:04d}.ckpt"  # Patch to which the checkpoints will be saved and their structure
...
roughDistances = [] # Initialising rough distances list for interpolation
################# Retrieving rough distances for 0 -> 360
for i in range(int(len(distances)/scanSize)):
    roughDistances.append(distances[scanSize*i:scanSize*(i+1)]) # Put all distances into one roughDistances list

for j in roughDistances:
    interpol = pd.Series(j) # turns the list into one-dimensional labelled data structure which can hold objects, strings or integers
    interpol.replace(0,np.NaN, inplace=True) # Replace zeros with numpy "NaN" as this is how the interpolation function requires the data to be structured to work
    x = interpol.interpolate(method = 'linear', limit_direction = 'both') # Use linear interpolation on the data, both ways because if list starts with NaN's it will skip them
    x = list(np.around(np.array(x),2)) # Converting back to a list
    finalDistances.append(x) # finalDistances
'''
################################################################
# Code used for debugging interpolation process
################################################################
...
balancingLength = len(shortestArray) # What is the size of the smallest actionBalancing part
...
################# This is where the train, validation and test datasets are finalised 
# using balanced indexes along with their corresponding labels
...
robotNet.compile(loss="sparse_categorical_crossentropy", # Calculate loss using cross entropy algorithm
...
robotNet.save_weights(checkpointPath.format(epoch = 0)) # Save weights in the directoryInitialise training process
...
#latest = tf.train.latest_checkpoint(checkpointDir) # find latest checkpoint
